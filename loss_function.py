...
class Tacotron2Loss(nn.Module):
    ...
    def forward(self, model_output, targets, re, batched_speakers):
        ...
        kl_loss = kld(Normal(means[0], stddevs[0]), re.p_zo_given_yo.distrib_lis[batched_speakers[0]]).mean()
        for i, speaker in enumerate(batched_speakers[1:], 1) :
            kl_loss += kld(Normal(means[i], stddevs[i]), re.p_zo_given_yo.distrib_lis[speaker]).mean()
#        print("speaker kl loss:- ", kl_loss/batched_speakers.shape[0])
        for i in range(re.p_zl_given_yl.n_disc) :
            ans = ( re.q_yl_given_X[i]*kld(re.q_zl_given_X_at_x, re.p_zl_given_yl.distrib_lis[i]).mean(dim=1) ).sum()
